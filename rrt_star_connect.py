...
class RRStarAlgo:
    ...
    def step(self):
        ...
        if self.is_final_node(new_node):
            ...
            self.try_connect_start(new_node)
            worked, node = self.try_connect_end(new_node)
            if worked:
                self.update_arrival_node(node)
    ...
    def insert_node(self, best_node: Node, new_node: Node):
        new_node.set_cost(best_node.get_cost() + Node.nodes_distance(best_node, new_node))
        best_node.add_node(new_node)
        self._nodes.append(new_node)

    def try_connect_end(self, new_node: Node):
        node = Node(self._final_position)
        if not self.does_line_intersect(node.get_position(), new_node.get_position()):
            node.set_cost(new_node.get_cost() + Node.nodes_distance(node, new_node))
    ...
    def try_connect_start(self, new_node: Node):
        if not self.does_line_intersect(self._tree.get_position(), new_node.get_position()):
            new_node.set_cost(Node.nodes_distance(self._tree, new_node))
            new_node.reconnect(self._tree)
